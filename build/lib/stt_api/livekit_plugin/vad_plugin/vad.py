...
class ProfilingStats:
    """Collected after profiling run or after stream ends when profiling=True."""
...
class VAD(agents.vad.VAD):
    ...
    def load(
        ...
        **Example:**
        ...
        main_session = onnx_model.new_inference_session(
            force_cpu, onnx_file_path=onnx_file_path or None
        )
        opts = _VADOptions(
        ...
        vad = cls(session=main_session, opts=opts)

        if profiling:
            stats = vad.run_profiling()
            logger.info("VAD profiling on load:\n%s", stats.summary())

        return vad

    # Number of sample frames to run during profiling warmup + benchmark
    PROFILING_WARMUP = 10
    ...
    def run_profiling(self, iterations: int | None = None) -> ProfilingStats:
        """Feed sample data directly to ONNX model and measure inference latency.

        Generates random float32 vectors matching the model's window size,
        runs them through the ONNX session, and collects timing stats.
        """
        n = iterations or self.PROFILING_ITERATIONS
        model = onnx_model.OnnxModel(
            onnx_session=self._session, sample_rate=self._opts.sample_rate
        )
        window = model.window_size_samples

        # Pre-generate sample inputs: mix of silence, noise, and speech-like patterns
        samples = []
        rng = np.random.default_rng(42)
        for i in range(self.PROFILING_WARMUP + n):
            if i % 3 == 0:
                # silence
                samples.append(np.zeros(window, dtype=np.float32))
            elif i % 3 == 1:
                # random noise (simulates background)
                samples.append(rng.uniform(-0.1, 0.1, size=window).astype(np.float32))
            else:
                # louder signal (simulates speech-like amplitude)
                samples.append(rng.uniform(-0.8, 0.8, size=window).astype(np.float32))

        # Warmup — don't measure
        for i in range(self.PROFILING_WARMUP):
            model(samples[i])
        ...
        for i in range(self.PROFILING_WARMUP, self.PROFILING_WARMUP + n):
...
class VADStream(agents.vad.VADStream):
    def __init__(self, vad: VAD, opts: _VADOptions, model: onnx_model.OnnxModel) -> None:
        ...
        # Live profiling: track every inference when profiling=True
    ...
    def stream_stats(self) -> ProfilingStats | None:
        """Live profiling stats collected during stream processing."""
    ...
    async def _main_task(self) -> None:
        ...
        async for input_frame in self._input_ch:
            ...
            while True:
                ...
                # run the inference (None = default executor, ONNX releases GIL)
                ...
                # Track live inference latency when profiling
        ...
        # Log live profiling stats when stream ends
        if self._profiling and self._stream_stats is not None and self._stream_start_time is not None:
            self._stream_stats.total_time = time.perf_counter() - self._stream_start_time
            logger.info("VAD stream profiling:\n%s", self._stream_stats.summary())