...
import yaml
import numpy as np
from ..utils.padding import sequence_1d
from ..utils import nemo_featurization
from ..utils.nemo_featurization import (
    AudioToMelSpectrogramPreprocessor,
)
from ..nemo import conv_asr
from ..nemo.conv_asr import SpeakerDecoder
...
class SpeakerVector(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        inputs = [
            input.array if isinstance(input, Frame) else input for input in inputs
        ]
...
class Classification(torch.nn.Module):
    def __init__(self, config, pth, label, model, name):
        ...
        preprocessor = d["preprocessor"].copy()
        preprocessor_target = (
            preprocessor.pop("_target_", None) or preprocessor.pop("cls", None)
        ).split(".")[-1]
        ...
        encoder = d["encoder"].copy()
        encoder_target = (
            encoder.pop("_target_", None) or encoder.pop("cls", None)
        ).split(".")[-1]
        ...
        decoder = d["decoder"].copy()
        decoder_target = (
            decoder.pop("_target_", None) or decoder.pop("cls", None)
        ).split(".")[-1]
        if "params" in decoder:
            decoder = decoder["params"]

        self.preprocessor = getattr(nemo_featurization, preprocessor_target)(
            **preprocessor
        )
        self.encoder = getattr(conv_asr, encoder_target)(**encoder)
