...
filename = hf_hub_download(
    repo_id="mesolitica/fasttext-language-detection-bahasa-en", filename="fasttext.ftz"
)
lang_model = fasttext.load_model(filename)
normalizer = malaya.normalize.normalizer()

logging.info("loading audio encoder")
if AUDIO_ENCODER == "neucodec":
    ...
    codebook_size = 50
elif AUDIO_ENCODER == "distilcodec":
    from distilcodec import DistilCodec

    if device == "cpu":
        raise RuntimeError("DistilCodec requires a GPU but was given CPU.")

    codec_model_config_path = hf_hub_download(
        repo_id="IDEA-Emdoor/DistilCodec-v1.0", filename="model_config.json"
    )
    codec_ckpt_path = hf_hub_download(
        repo_id="IDEA-Emdoor/DistilCodec-v1.0", filename="g_00204000"
    )

    codec = (
        DistilCodec.from_pretrained(
    ...
    raise ValueError(
        "`AUDIO_ENCODER` os environment only support `neucodec` or `distilcodec`."
    )
...
def fn(padded_token):
    if AUDIO_ENCODER == "neucodec":
        ...
        # For distilcodec, loop over batch since decode_from_codes is not natively batched
        results = []
        for i in range(padded_token.shape[0]):
            row = padded_token[i]
            # Remove padding
            non_zero = (row != 0).nonzero(as_tuple=True)[0]
            if len(non_zero) > 0:
                length = non_zero[-1].item() + 1
                codes = row[:length].tolist()
            else:
                codes = []
            result = codec.decode_from_codes(codes, minus_token_offset=False)
            results.append(
                result.squeeze(0)
            )  # Assuming result is (1, sr * len(codes)/codebook_size)
        # Stack results
    ...
    for B in tqdm(BUCKET_BATCHES):
        for T in BUCKET_TOKENS:
            ...
            if TORCH_COMPILE:
                ...
                fn_with_graph = CUDAGraphsWrapper.wrap(
                    fn, [input], stream=compute_stream
                )
        ...
        while not dynamic_batch_queue.empty():
            try:
                request = await asyncio.wait_for(
                    dynamic_batch_queue.get(), timeout=1e-9
                )
...
@app.get("/v1/audio/pronunciation")
...
async def tts_stream(data: TTSRequest, request: Request = None):
    ...
    if data.normalize:
        ...
        if s != original_s:
            logging.debug(
                f'Pronunciation replacements applied: "{original_s}" -> "{s}"'
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        ...
                        logging.error(
                            f"TTS backend error: {resp.status} - {error_text}"
                        )
                        return
...
t1 = threading.Thread(target=batch_thread_fn, daemon=True)
t1.start()
t2 = threading.Thread(target=compute_thread_fn, args=(loop,), daemon=True)
t2.start()
